/**
 * Quick runner for HTTP staged scenarios against verify().
 * Run: bun scripts/harness/run-http-test.ts
 *
 * These test the GROUNDING gate for HTTP predicates — no live server needed.
 * Gates disabled: staging, browser, http (the actual HTTP gate), invariants, vision
...
const scenarioPath = resolve('fixtures/scenarios/http-staged.json');
const scenarios = JSON.parse(readFileSync(scenarioPath, 'utf-8'));

console.log(`Loaded ${scenarios.length} HTTP scenarios\n`);

let pass = 0, fail = 0, errCount = 0;
const failures: any[] = [];

for (const s of scenarios) {
  const stateDir = join(tmpdir(), `verify-http-${s.id}`);
  try {
    ...
    const config = {
      ...
      gates: {
        staging: false,
        browser: false,
        http: false,       // Disable the HTTP gate (needs live server)
        invariants: false,
        vision: false,
    ...
    if (actual === expected) {
      ...
      failures.push({
        ...
        detail: failedGate?.detail?.slice(0, 150) || '',
    ...
    errCount++;
    failures.push({ id: s.id, description: s.description, error: e.message?.slice(0, 150), tags: s.tags });
  } finally {
...
for (const [type, r] of Object.entries(typeResults).sort((a, b) => b[1].fail - a[1].fail)) {
  ...
  console.log(`  ${type.padEnd(20)} ${r.pass}/${total} pass (${pct}%) — ${r.fail} failures`);
}
