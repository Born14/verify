/**
 * Quick runner for K5 staged scenarios against verify().
 * K5 scenarios need pre-seeded constraint state in their stateDir.
 * Run: bun scripts/harness/run-k5-test.ts
 */
...
const scenarioPath = resolve('fixtures/scenarios/k5-staged.json');
const scenarios = JSON.parse(readFileSync(scenarioPath, 'utf-8'));

console.log(`Loaded ${scenarios.length} K5 scenarios\n`);

let pass = 0, fail = 0, errCount = 0;
const failures: any[] = [];

for (const s of scenarios) {
  const stateDir = join(tmpdir(), `verify-k5-${s.id}`);
  try {
    mkdirSync(stateDir, { recursive: true });

    // Pre-seed constraints if provided
    if (s.seedConstraints && s.seedConstraints.length > 0) {
      const compactEntry = {
        _op: 'compact',
        _ts: Date.now(),
        data: {
          constraints: s.seedConstraints,
          outcomes: [],
          patterns: [],
        },
      };
      writeFileSync(join(stateDir, 'memory.jsonl'), JSON.stringify(compactEntry) + '\n');
    }
