/**
 * Quick runner for content staged scenarios against verify().
 * Run: bun scripts/harness/run-content-test.ts
 */
...
const scenarioPath = resolve('fixtures/scenarios/content-staged.json');
const scenarios = JSON.parse(readFileSync(scenarioPath, 'utf-8'));

console.log(`Loaded ${scenarios.length} content scenarios\n`);

let pass = 0, fail = 0, errCount = 0;
const failures: any[] = [];

for (const s of scenarios) {
  const stateDir = join(tmpdir(), `verify-content-${s.id}`);
  try {
